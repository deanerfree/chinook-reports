defmodule ChinookReportsWeb.CoreComponents do
  @moduledoc """
  ...
  def modal(assigns) do
    ...
    <dialog
      ...
      data-cancel={@on_cancel |> hide_modal(@id)}
      class="fixed inset-0 z-200 m-0 w-screen h-screen max-w-none max-h-none border-0 p-0 bg-transparent open:flex items-center justify-center"
    >
      <%!-- backdrop: blurs and dims the page behind the modal --%>
      <div class="absolute inset-0 bg-zinc-900/60 backdrop-blur-sm" aria-hidden="true" />

      <%!-- content: centered card with close button floating outside top-right --%>
      <div class="relative z-10 mx-4 my-8 w-full max-w-lg">
        <button
          phx-click={JS.exec("data-cancel", to: "##{@id}")}
          type="button"
          class="absolute -top-4 -right-4 z-20 flex h-9 w-9 items-center justify-center rounded-full bg-white shadow-md text-copy-secondary hover:text-copy transition-colors"
          aria-label={gettext("close")}
        >
          <.icon name="hero-x-mark-solid" class="h-4 w-4" />
        </button>
        <div id={"#{@id}-content"} class="rounded-2xl bg-white shadow-xl ring-1 ring-black/8 p-10">
  ...
  def table(assigns) do
    ...
    ~H"""
    <div class="border-2 border-border-light rounded-lg overflow-hidden" style={@style}>
      <table class="w-full py-4">
        <thead
          class="text-sm text-left leading-6"
          style="background-color: var(--color-accent-subtle); color: var(--table-header-color, #71717a)"
        >
          <tr>
            <th :for={col <- @col} class="px-2 py-1 font-semibold text-primary">{col[:label]}</th>
        ...
        <tbody
          ...
          class="relative divide-y w-full divide-zinc-100 border-t border-zinc-200 leading-6 text-zinc-700"
          style="font-size: var(--table-font-size)"
        >
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-zinc-50 px-4">
            <td
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right text-xs font-medium">
                ...
                <span
                  :for={action <- @action}
                  class="relative ml-4 font-semibold leading-6 text-zinc-900 text-sm hover:text-zinc-700"
                >
  ...
  def show_modal(js \\ %JS{}, id) when is_binary(id) do
    js
    |> JS.dispatch("dialog:open", to: "##{id}")
    |> JS.add_class("overflow-hidden", to: "body")
  end

  def hide_modal(js \\ %JS{}, id) do
    js
    |> JS.dispatch("dialog:close", to: "##{id}")
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ChinookReportsWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ChinookReportsWeb.Gettext, "errors", msg, opts)
    end
  end
